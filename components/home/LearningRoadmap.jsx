        ...
        <Reveal>
          <div className="mb-9 max-w-2xl">
            ...
            <h2 className="text-2xl font-black tracking-tight text-[var(--av-text)] sm:text-3xl">
              هر گام، لایه‌ای تازه از دبیره را آشکار می‌کند.
            </h2>

            <p className="mt-2 text-xs leading-7 text-[var(--av-text-secondary)] sm:text-sm">
              یادگیری از شناخت شکل نویسه آغاز می‌شود و با بازیابی فعال و مرور
              فاصله‌دار به یادسپاری بلندمدت می‌رسد.
            </p>
          </div>
        </Reveal>
        ...
        <div className="relative grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          {roadmapSteps.map((item, index) => (
            <Reveal key={item.step} delay={index * 0.05}>
              ...
              >
                <span className="mb-6 flex h-7 w-7 items-center justify-center rounded-full border border-[var(--av-brand)]/20 bg-[var(--av-brand-soft)] text-[10px] font-black text-[var(--av-brand)]">
                  {item.step}
                </span>

                <h3 className="text-xs font-black text-[var(--av-text)]">
                  {item.title}
                </h3>

                <p className="mt-2 text-[10px] leading-6 text-[var(--av-text-secondary)]">
                  {item.description}
                </p>
              </motion.article>
            </Reveal>
